# 📱 GateVue App (Visitor Management System)

## 🚀 Overview

This is a React Native mobile application inspired by a real-world visitor management system (GateVue). The app replicates core functionalities such as authentication, role-based navigation, and visitor tracking.

## 🎯 Objective

To build a real-world mobile application with multiple user roles and dynamic navigation using React Native.

## ✨ Features

* 🔐 User Login (API integrated)
* 👥 Role-Based Access:

  * Admin
  * Security
  * Super Admin
* 🧭 Dynamic Navigation based on roles
* 📋 Visitor Management
* 📊 Dashboard & Reports

## 🧠 Implementation Highlights

* Role-based navigation using Context API
* Persistent login using AsyncStorage
* API handling using Axios
* Clean and modular folder structure

## 🛠 Tech Stack

* React Native
* JavaScript
* Axios
* AsyncStorage
* React Navigation
## 📸 Screenshots

### 🔐 Login Screen
![Login](./Login_Screen.png)

### 🏠 Dashboard
![Dashboard](./HomeScreen.png)

### 📋 Visitors Screen
![Visitors](./Visitors_Screen.png)

### 👤 Admin Panel
![Admin](./Add_AdminUser.png)

### Navigation Bar
![Navigation bar](./Navigation_Bar.png)

### Security Users Screen
![Security](./SecurityUsers_Screen.png).

###Reports Screen
![Reports](./Reports_Screen.png)





## ⚙️ Installation

```bash
npm install
npx react-native run-android
```

## 📂 Project Structure

src/
 ├── assets/        # Images and icons
 ├── components/    # Reusable UI components
 ├── screens/       # App screens
 ├── navigation/    # Navigation setup
 ├── services/      # API calls (Axios)
 ├── context/       # Global state management
 ├── constants/     # App constants
 └── hooks/         # Custom hooks (future use)

## 💡 Learnings

* Role-based navigation implementation
* API integration in mobile apps
* State management using Context API
* Building scalable React Native apps

## 🎯 Why I Built This
To understand real-world mobile app architecture, role-based navigation, and API integration using React Native.

## 🚀 One-Line Summary
A React Native app replicating a real-world visitor management system with role-based access control and API integration.  

## 👩‍💻 Author

Sindhu
